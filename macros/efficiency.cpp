...
void efficiency()
{
    ...
    bool switchStation = true;
    bool connectAnything = false;
    int nbEvents_for = 1e6;

    double MergingErrorRate = 0.1;
    ...
    if (efficiencyModules.size() != 24) return;
...
void accessAndSaveEfficiencies(const char* fileName, vector<TGraphAsymmErrors>& efficiencies) {
    ...
    while ((obj = next())) {
        if (obj->IsA() == TPad::Class()) {
            ...
            TIter next2(listOfPrimitives2);  // Correction ici
